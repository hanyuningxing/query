...
import com.miracle.lotteryutils.MathUtils;
import com.miracle.lotteryutils.PassTypeItem;

/**
 * 竞彩篮球过关方式
 * 
 */
public enum JclqPassType implements PassTypeItem {
	/** 单关 */
	P1(1, new int[] { 1 },"01") {
	},
	 ...
	 */
	private JclqPassType(int matchCount, int[] passMatchs,String cpText) {
	...
	public static List<JclqPassType> findPassTypes(int pass, int maxMatchSize) {
		List<JclqPassType> list = new ArrayList<JclqPassType>();
		for (JclqPassType passType : JclqPassType.values()) {
	...
	public static List<JclqPassType> findPassTypes(List<Integer> passList, int maxMatchSize) {
		List<JclqPassType> list = new ArrayList<JclqPassType>();
		for (JclqPassType passType : JclqPassType.values()) {
	 ...
	 * @return 过关方式类型
	 */
	public static JclqPassType valueOfType(String typeName) {
		if (StringUtils.isNotBlank(typeName)) {
			for (JclqPassType spPassType : values()) {
	 ...
	 * @return 过关方式类型
	 */
	public static JclqPassType getPassType(long value) {
		for (JclqPassType spPassType : values()) {
	 ...
	 * @return 过关方式类型集合
	 */
	public static List<JclqPassType> getPassTypes(long mixPtValue) {
		List<JclqPassType> sets = Lists.newArrayList();
		for (JclqPassType spPassType : values()) {
