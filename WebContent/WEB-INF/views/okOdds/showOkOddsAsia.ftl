      	...
      	<#if asiaList?? && asiaList?size gt 0>
	      	<tr class="tdtitle">
      		  ...
      		  <tr class="tdtitle">
	      			 ...
	      			 <td class="doc_td_other">主</td>
	      			 <td class="doc_td_other">客</td>
	      			 <td class="doc_td_other">值</td>
	      		</tr>  
	      	<#list  asiaList as odds>
	      	 	<#if (odds_index+1)%2==0><#assign classStr='tdwhitelist' /><#else><#assign classStr='tdgraylist' /></#if>
	      		<tr name="odds_${odds.fetchOddsKey}" <#if odds_index=0 >id="odds_99" <#if odds.match??> time="${odds.match.matchTime?string("yyyy-MM-dd HH:mm:ss")}"</#if></#if> class="${classStr}"  onmouseover="this.className='trhover'" onmouseout="this.className='${classStr}'" >
	      			...
	      			<td><span  onmouseover="showTimeTips(this);"  name="lastModifyTime" tips="" data="" <#if odds.lastModifyTime??>time="${odds.lastModifyTime?string("yyyy-MM-dd HH:mm:ss")}"</#if>><em class="time_5"></em></span></td>
